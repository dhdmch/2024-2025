...
- Raccolta e strutturazione di alcuni dati in una tabella CSV (es. ogni riga rappresenta una chiesa, e ogni colonna una sua caratteristica) e pulizia dei dati usando OpenRefine ([esempio parziale di tabella](esempi/esempio-tabella.md). Per raccogliere i dati potete usare qualsiasi software (anche Excel), basta che poi la tabella sia esportata in CSV. Idealmente, dovreste anche spiegare perché avete strutturato la tabella in quel modo, quali buone pratiche avete seguito (separatori, schemi di codifica, ecc.). Non devono essere necessariamente standard internazionali, ma se seguite uno schema preciso (es. "Cognome, Nome" per le persone), dovete specificarlo nella documentazione e seguirlo in maniera sistematica;
...
- Modellazione semantica dei dati in RDF, creando degli esempi di modellazione in un file Turtle riusando ontologie esistenti, rappresentando entità (persone, monumenti, eventi, ecc.) e le loro proprietà (sia relazioni che attributi) ([esempio parziale di modellazione](esempi/esempio-rdf.ttl). Idealmente, dovreste indicare gli elementi che avete estratto dal testo (quali classi? quali proprietà?), i vocabolari che avete deciso di riutilizzare e come li avete riutilizzati.
